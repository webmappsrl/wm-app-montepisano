...
yargs
  ...
  .command(
    "build",
    "Create, Update and generate Ionic platforms and resources  of the istance of Webmapp"
  )
  .command("create", "Create the istance of Webmapp")
  .example(
    "$0 create -i webmapp_demo_app -u http://pnfc.j.webmapp.it/",
    "Create the istance of Webmapp"
  )
  .command("update", "update the istance of Webmapp")
  .command("update", "update the istance of Webmapp")
  .example(
    "$0 update -i webmapp_demo_app -u http://pnfc.j.webmapp.it/",
    "update the istance of Webmapp"
  )
  .command(
    "set",
    "set config.js in bare/index.html e con -c recupera config.js da url remoto"
  )
  .example(
    "$0 config -i webmapp_demo_app -c http://pnfc.j.webmapp.it/config.js",
    "update the istance of Webmapp"
  )
  .command(
    "update-instance",
    "aggiorna il core dell'istanza con i files del core"
  )
  .demandCommand(1, "")
...
gulp.task("node_modules_link", function () {
  return gulp.src("bare/node_modules").pipe(
    symlink("instances/" + instance_name + "/node_modules", {
      force: true,
    })
  );
});
      ...
      .pipe(
        streamify(
          jeditor(function (repositories) {
            ...
            gulp
              .getUrlFile("config.js", config, dir + "/www/config/")
              .on("end", function () {
...
gulp.setFont = function (fontFamily) {
  ...
  return gulp
    .src(dir + "/_variables.scss")
    .pipe(
      replace(
        /\$main-font-family: '(.*)', sans-serif;/g,
        "$main-font-family: '" + fontFamily + "', sans-serif;"
      )
    )
    .pipe(
      replace(
        /\$title-font-family: '(.*)', sans-serif;/g,
        "$title-font-family: '" + fontFamily + "', sans-serif;"
      )
    )
    .pipe(gulp.dest(dir));
};
...
gulp.getChanges = function (oldVersion, newVersion) {
  sh.exec(
    "git log --pretty=oneline --abbrev-commit BETA_" +
      oldVersion +
      "...BETA_" +
      newVersion +
      " > tmp.txt"
