angular
  ...
  .controller(
    ...
    function DetailController(
      ...
      $ionicModal
        .fromTemplateUrl(templateBasePath + "js/modals/tableModal.html", {
          scope: modalScope,
      ...
      $ionicModal
        .fromTemplateUrl(templateBasePath + "js/modals/featureListModal.html", {
          scope: modalScope,
          animation: "slide-in-up",
        })
        .then(function (modalObj) {
      ...
      $ionicModal
        .fromTemplateUrl(templateBasePath + "js/modals/eventListModal.html", {
          scope: modalScope,
          animation: "slide-in-up",
        })
        .then(function (modalObj) {
      ...
      $ionicModal
        .fromTemplateUrl(templateBasePath + "js/modals/imagesModal.html", {
          scope: modalScope,
          animation: "slide-in-up",
        })
        .then(function (modalObj) {
      ...
      $ionicModal
        .fromTemplateUrl(templateBasePath + "js/modals/textModal.html", {
          scope: modalScope,
          animation: "slide-in-up",
        })
        .then(function (modalObj) {
      ...
      $ionicModal
        .fromTemplateUrl(templateBasePath + "js/modals/couponsModal.html", {
          scope: modalScope,
          animation: "slide-in-up",
        })
        .then(function (modalObj) {
      ...
      $ionicModal
        .fromTemplateUrl(templateBasePath + "js/modals/detailModal.html", {
          scope: modalScope,
          animation: "slide-in-up",
        })
        .then(function (modalObj) {
      ...
      $ionicModal
        .fromTemplateUrl(
          templateBasePath + "js/modals/accessibilityModal.html",
          {
            scope: modalScope,
            animation: "slide-in-up",
          }
      ...
      var buildDetail = function (data) {
        ...
        if (typeof mappingUrls !== "undefined") {
          for (var u in mappingUrls) {
            if (typeof _feature[u] !== "undefined" && _feature[u] !== null) {
        ...
        if (feature && parent) {
          ...
          for (var g in feature.imageGallery) {
            ...
            vm.imageGallery.push({
              ...
              caption: feature.imageGallery[g].caption
                ? feature.imageGallery[g].caption
                : "",
            });
          }
          ...
          if (feature.description) {
            ...
            feature.description = feature.description.replace(
              new RegExp(/src="\//g),
              'src="' + CONFIG.COMMUNICATION.baseUrl
            );
          ...
          if (typeof feature.events === "object" && feature.events !== null) {
            vm.relatedEvents = [];
            for (var z in feature.events) {
              MapService.getEventById(feature.events[z]).then(function (evt) {
          ...
          if (typeof feature.coupons === "object" && feature.coupons !== null) {
            vm.relatedCoupons = [];
            for (var o in feature.coupons) {
              MapService.getCouponById(feature.coupons[o]).then(function (evt) {
          ...
          if (typeof feature.stages === "object" && feature.stages !== null) {
            vm.stages = [];
            for (var s in feature.stages) {
              vm.stages[s] = angular.extend({}, feature.stages[s]);
              vm.stages[s].pois = [];
              for (var p in feature.stages[s].pois) {
                MapService.getFeatureById(feature.stages[s].pois[p]).then(
                  $.proxy(
                    function (data) {
          ...
          if (
            ...
            feature.id_pois &&
            feature.id_pois.length
          ) {
            if (
              !(
                CONFIG.OPTIONS.hideRelatedPoiInDetails ||
                (CONFIG.MAIN &&
                  CONFIG.MAIN.OPTIONS.hideRelatedPoiInDetails &&
                  typeof CONFIG.OPTIONS.hideRelatedPoiInDetails == "undefined")
              )
            ) {
          ...
          if (feature.ricette) {
            vm.related = MapService.getRelatedRicetteById(feature.ricette);
          }

          vm.relatedItinerary =
            MapService.getItineraryRefByFeatureIdMap()[feature.id] || [];
          vm.feature = feature;
          vm.geometry = data.geometry;
          vm.coordinates = data.geometry
        ...
        setTimeout(function () {
          ...
          if (extras.length > 0) objData.extras = extras;

          MapService.addFeaturesToFilteredLayer(objData, true);

          if (centerOnPoint) {
            MapService.centerOnCoords(
        ...
        if (vm.feature.accessibility) {
          ...
          for (var i in vm.feature.accessibility) {
            if (
              !vm.feature.accessibility[i].description ||
              vm.feature.accessibility[i].description === ""
            ) {
              vm.feature.accessibility[i].description =
                "Questa descrizione non è ancora stata inserita e sarà disponibile a breve.";
            }
          }
        }

        if (vm.featureDetails.menu) vm.showMenu = true;
      ...
      vm.getTaxonomyName = function (id) {
      ...
      modalScope.vm.openFeature = function (feature) {
        Utils.goTo(
          "layer/" +
            feature.parent.label.replace(/ /g, "_") +
            "/" +
      ...
      modalScope.vm.nextImage = function () {
        if (
          ...
          $ionicSlideBoxDelegate._instances[
            $ionicSlideBoxDelegate._instances.length - 1
          ].next();
        }
      };
      ...
      if (current.name === "app.main.detaillayer") {
        MapService.getFeatureById(
          params.id,
          params.parentId.replace(/_/g, " ")
        ...
        MapService.getAreaById(params.id).then(
          function (data) {
            buildDetail(data);
            setTimeout(function () {
        ...
        MapService.getEventById(params.id).then(
          function (evt) {
            var event = angular.extend({}, evt);
            ...
            if (event.body) {
              ...
              vm.mainDescription.html = $sce.trustAsHtml(
                vm.mainDescription.html
              );
            ...
            for (var i in event.pois) {
              MapService.getFeatureById(event.pois[i]).then(function (data) {
      ...
      vm.openRelatedUrlPopup = function () {
        if (vm.feature.web && typeof vm.feature.web === "string") {
          ...
          vm.feature.facebook &&
          typeof vm.feature.facebook === "string"
        ) {
      ...
      vm.openRelatedPlaces = function () {
        ...
        for (var i in vm.relatedPlaces) {
          if (
            vm.relatedPlaces[i].properties &&
            vm.relatedPlaces[i].properties.id &&
            featureById[vm.relatedPlaces[i].properties.id] !== "undefined"
          ) {
            modalScope.vm.featureList.push(
              featureById[vm.relatedPlaces[i].properties.id]
            );
          }
        }
      ...
      document.addEventListener(
        ...
        function () {
          isOnline =
            Connection && navigator.network.connection.type !== Connection.NONE;
        },
        false
      );
