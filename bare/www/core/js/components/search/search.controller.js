...
angular
  ...
  .controller(
    ...
    function SearchController(
        ...
        for (var i in array) {
          ...
          if (translatedName !== "") {
            for (var pos in CONFIG.OVERLAY_LAYERS) {
              if (
                CONFIG.OVERLAY_LAYERS[pos].label === translatedName &&
      ...
      vm.translatedFiltersList = vm.translateOverlayInArray(
        vm.translatedFiltersList
      );
      ...
      modalScope.vm.updateFilter = function (filterName, value) {
        if (modalScope.vm.isNewModal) {
          ...
          MapService.addFeaturesToFilteredLayer(
            Search.getByLayersGroupedByLayer(lastQuery)
          );
          ...
          for (var i in vm.results) {
            ...
            if (
              !hasGeometries &&
              !!vm.results[i].geometry &&
              !!vm.results[i].geometry.type &&
              !!vm.results[i].geometry.coordinates
            )
              hasGeometries = true;
          }
          vm.disableMapButton = !hasGeometries;
          if (vm.disableMapButton) vm.toggleMap();
        } else {
          ...
          vm.translatedFiltersList = vm.translateOverlayInArray(vm.filtersList);
          vm.othersCount = String(vm.filtersList.length - 1);
          vm.areAllActive = modalScope.vm.areAllActive = areAllActive(
            Search.getActiveLayersMap()
          );
          vm.results = vm.translateOverlayInArray(
            Search.getByLayersWithDivider(lastQuery, vm.filtersList)
          );
          MapService.addFeaturesToFilteredLayer(
            Search.getByLayersGroupedByLayer(lastQuery, vm.filtersList)
          );
        }
      ...
      vm.updateSearch = function (query) {
        vm.results = vm.translateOverlayInArray(
          Search.getByLayersLexicalOrder(query, Search.getActiveLayers())
        );
        updateClickableCheckBoxes(query);
        vm.results.realLength = 0;
        ...
        MapService.addFeaturesToFilteredLayer(
          Search.getByLayersGroupedByLayer(query, Search.getActiveLayers())
        );
        $ionicScrollDelegate.scrollTop();
        lastQuery = query;
      };
      ...
      vm.toggleMap = function () {
        if (!vm.disableMapButton) vm.isMapView = !vm.isMapView;
        else vm.isMapView = false;
      ...
      if (modalScope.vm.isNewModal) {
        ...
        modalScope.toggleLayer = function (layerLabel, sublayerId, tabId) {
          if (
            modalScope.layers[layerLabel].clickable &&
        ...
        var updateClickableCheckBoxes = function (query) {
          var currentQuery = typeof query === "undefined" ? lastQuery : query;

          for (var label in modalScope.layers) {
            modalScope.layers[label].clickable = true;
          }

          var orginalFilters = Search.getActiveLayers();
          var prevFilters = Search.getActiveLayers();
          var additionalFilter = Search.searchForQueryFilters(currentQuery);
          var updatedQuery = Search.updateQuery(
            currentQuery,
            additionalFilter.wordsToRemove
          );

          var filterMap = Search.getActiveLayersMap();

          for (const key in filterMap) {
            if (prevFilters.indexOf(key) == -1 && additionalFilter[key]) {
          ...
          for (var type in modalScope.filters) {
            if (type !== "base_maps") {
              ...
              for (var i = 0; i < superCategory.sublayers.length; i++) {
                ...
                var results = Search.getFeatures(updatedQuery);

                let mapping = {
                  content_types: "tipo",
                  places: "localita",
                  tags: "tags",
                  recipe_categories: "categorie-ricette",
                };

                for (let k = 0; k < results.length; k++) {
                  var layer = results[k];
                  if (layer.properties && layer.properties.taxonomy) {
                    if (!!macroCategory.id && !!mapping[macroCategory.id]) {
                      var typeArray =
                        layer.properties.taxonomy[mapping[macroCategory.id]];
                ...
                for (var catId in categoriesContainerMap) {
                  ...
                  if (layer && modalScope.layers[layer.label]) {
                    if (!modalScope.layers[layer.label].checked) {
                      modalScope.layers[layer.label].clickable =
                        categoriesContainerMap[catId];
                    }
                  }
                }
              }
          ...
          Search.setActiveLayers(orginalFilters);
        };
      }
