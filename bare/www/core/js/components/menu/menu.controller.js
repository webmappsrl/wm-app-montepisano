angular.module('webmapp')
    ...
    .controller('MenuController', function MenuController(
        ...
        $rootScope.$on('track-loaded', function (e, value) {
            var track = value,
                url = 'layer/' + track.parent.label.replace(/ /, "_") + '/' + track.properties.id,
                label = track.properties.name;

            for (var i in vm.advancedMenuItems) {
                if (vm.advancedMenuItems[i].type === 'layer') {
                    vm.advancedMenuItems[i].url = url;
                    vm.advancedMenuItems[i].label = label;
                    Utils.forceDigest();
                    break;
                }
            }
