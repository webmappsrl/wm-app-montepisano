...
angular
  ...
  .factory("Model", function Model($http, $rootScope, CONFIG, Utils, Search) {
    var model = {};
    ...
    model.buildItemUrl = function (item) {
      var realUrl = item.label.replace(/ /g, "_"),
        realType = item.type,
        separator = "/";
    ...
    model.addItemToLayer = function (item, layer) {
      if (
        ...
        if (!layer.skipSearch) Search.addToIndex(item, layer.label);
