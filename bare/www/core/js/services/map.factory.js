  ...
  .factory(
    ...
    function MapService(
      ...
      var eventsList = [],
        ...
        centerCoords = {},
        centerCoordsUTM32 = {},
        singleFeatureUrl = CONFIG.COMMUNICATION
          ? CONFIG.COMMUNICATION.singleFeatureUrl
          : null,
        eventsPromise,
        couponsPromise,
        pagePromise,
      ...
      var overlayLayersConfMap = overlayLayersConf.reduce(function (
        prev,
        curr
      ) {
      ...
      mapService.getLayerLabelById = function (id, forceLabel) {
        currentLang = currentLang = $translate.preferredLanguage()
          ? $translate.preferredLanguage()
          : "it";
        if (overlayLayersById[id].languages && !forceLabel) {
          return (
            overlayLayersById[id].languages[currentLang] ||
            overlayLayersById[id].languages[defaultLang] ||
            overlayLayersById[id].languages[
              Object.keys(overlayLayersById[id].languages[0])
            ] ||
            overlayLayersById[id].label
          );
        } else return overlayLayersById[id].label;
      };

      var activeFilters = localStorage.activeFilters
        ? JSON.parse(localStorage.activeFilters)
        : overlayLayersConf.reduce(function (prev, curr) {
            if (!curr.skipRendering) {
              if (
                curr.label === "Ristoranti" ||
                curr.label === "Botteghe" ||
                curr.label === "Eventi" ||
                curr.label === "Produttori"
              )
                prev[curr.label] = curr.showByDefault !== false;
              else prev[curr.label] = false;
            }
            return prev;
          }, {});
      ...
      var isAPOILayer = function (layerName) {
        return (
          (overlayLayersConfMap[layerName] &&
      ...
      var getLayerByName = function (layerName) {
        return overlayLayersByLabel[layerName] || extraLayersByLabel[layerName];
      };
      ...
      var addLayer = function (layerName) {
        ...
        if (layer) {
          if (isAPOILayer(layerName)) markerClusters.addLayer(layer);
          else if (isALineLayer(layerName)) {
            map.addLayer(layer);

            if (
              generalConf.showArrows ||
              (CONFIG.MAIN && CONFIG.MAIN.OPTIONS.showArrows)
            ) {
              if (layerName === "filteredLine") {
                for (var i in layer.arrows) {
                  polylineDecoratorLayers[layer.arrows[i].parentLabel][
                    layer.arrows[i].featureId
                  ].addTo(map);
                }
              } else {
            ...
            if (generalConf.useAlmostOver) map.almostOver.addLayer(layer);
      ...
      var removeLayer = function (layerName) {
        ...
        if (layer) {
          if (isAPOILayer(layerName)) markerClusters.removeLayer(layer);
          else if (isALineLayer(layerName)) {
            ...
            if (generalConf.useAlmostOver) map.almostOver.removeLayer(layer);
          } else if (utfGridOverlayLayersByLabel[layerName]) {
            map.removeLayer(layer);
      ...
      var globalLineApplyStyle = function (feature) {
        ...
        return {
          color:
            ...
            overlayConf.color ||
            styleConf.line.default.color,
          weight:
            feature.properties.weight ||
            overlayConf.weight ||
            styleConf.line.default.weight,
          opacity:
            feature.properties.opacity ||
            overlayConf.opacity ||
            styleConf.line.default.opacity,
        };
      };
      ...
      var globalOnEachPOI = function (feature) {
        ...
        feature.properties.color =
          feature.properties.color || overlayConf.color;
        ...
        if (feature.parent && feature.parent.id) {
          ...
          featuresIdByLayersMap[feature.parent.label].push(
            feature.properties.id
          );

          if (feature.properties.taxonomy) {
            let singleTaxonomy = ["tipo", "localita"],
              arrayTaxonomy = ["categorie-ricette", "tags"];

            for (let i of singleTaxonomy) {
              if (
                feature.properties.taxonomy[i] &&
                feature.properties.taxonomy[i].length > 0
              ) {
                var categoryId = feature.properties.taxonomy[i][0];
                ...
                if (category && category.label) {
                  ...
                  featuresIdByLayersMap[category.label].push(
                    feature.properties.id
                  );
                }
              }
        ...
        if (feature.properties.stages) {
          for (var i in feature.properties.stages) {
            for (var j in feature.properties.stages[i].pois) {
              ...
              if (
                typeof itineraryRefByFeatureId[currentRelatedPOIs] ===
                "undefined"
              )
                itineraryRefByFeatureId[currentRelatedPOIs] = [];
              if (
                itineraryRefByFeatureId[currentRelatedPOIs].indexOf(
                  feature.properties.id
                ) === -1
              )
                itineraryRefByFeatureId[currentRelatedPOIs].push(
                  feature.properties.id
                );
            }
          }
      ...
      var getFeatureIcon = function (feature, overlayLayer) {
        return (
          feature.properties.icon || overlayLayer.icon || "wm-icon-generic"
        );
      };
      ...
      var activatePopup = function (e, isPOI) {
        ...
        if (
          ...
          if (interaction) {
            var content =
              '<div class="popup-div" onclick="goToDetail(\'' +
            ...
            if (e.layer.feature.properties.image) {
              content =
                ...
                '<div class="popup-img">' +
                '<img src="' +
                Offline.getRealImageUrl(e.layer.feature.properties.image) +
                '" />' +
                "</div>";
              ...
              content =
                ...
                '<div class="popup-img">' +
                '<img src="' +
                Offline.getRealImageUrl(
                  e.layer.feature.properties.imageGallery[0].src
                ) +
                '" />' +
                "</div>";
            } else {
              content =
                ...
                "<div>" +
                '<i class="icon ' +
                e.layer.feature.properties.icon +
                '"></i>' +
                "</div>" +
                "</div>";
            }
            ...
            if (
              category.toLowerCase() === "tappe" ||
              category.toLowerCase() === "stages"
            )
              category = $translate.instant("Tappe");
            else if (
              e.layer.feature.parent.languages &&
              e.layer.feature.parent.languages[currentLang]
            )
            ...
            L.popup({
              ...
              .setLatLng({
                lat:
                  e.latlng.lat +
                  (isPOI && !mapIsRotating ? getIncrement(map.getZoom()) : 0),
                lng: e.latlng.lng,
              })
              .setContent(content)
              .openOn(map);
          ...
          var content =
            '<div class="popup-div" onclick="goToTileUtfGridDetail(\'' +
            e.data.id +
            "', '" +
            e.parent.label +
            "', '" +
      ...
      var activatePOIHandlers = function (pointsLayer) {
        pointsLayer.on("click", function (e) {
          if (isPOILayerDetail()) return;

          activatePopup(e, true);
        });
      };

      var addFeaturesToFilteredLayer = function (
        groupedFeatures,
        fitToBounds,
        delayedFit
      ) {
        ...
        for (var i in groupedFeatures) {
          for (var j in groupedFeatures[i]) {
            if (
              typeof groupedFeatures[i][j] !== "undefined" &&
              typeof groupedFeatures[i][j].properties !== "undefined"
            ) {
              ...
              if (typeof featureMapById[currentId] !== "undefined") {
                ...
                if (!!feature.geometry) {
                  ...
                  else linesCollection.features.push(feature);
                }
              }
            }
          }
      ...
      var initializeEvents = function () {
        ...
        if (CONFIG.EXTRA && CONFIG.EXTRA.events) {
          var currentFromLocalStorage = localStorage.getItem(
            CONFIG.EXTRA.events.serverUrl
          );
          ...
          var eventCallback = function (data) {
            ...
            if (queueEvents) mapService.showEventsLayer();
      ...
      var initializePages = function () {
          ...
          if (item.isCustom) {
            if (CONFIG.LANGUAGES && CONFIG.LANGUAGES.available) {
              for (var pos in CONFIG.LANGUAGES.available) {
                ...
                if (
                  CONFIG.LANGUAGES.available[pos].substring(0, 2) !==
                  CONFIG.LANGUAGES.actual.substring(0, 2)
                )
                  url =
                    url + "_" + CONFIG.LANGUAGES.available[pos].substring(0, 2);
        ...
        if (pages) pages.forEach(requestPages);
      };
      ...
      var initializeThen = function (currentOverlay) {
        ...
        if (group) {
          ...
          } else if (queueLayerToActivate === group.label)
      ...
      var initializeLayer = function (currentOverlay) {
        if (
          typeof overlayLayersQueueByLabel[currentOverlay.label] !== "undefined"
        )
          return overlayLayersQueueByLabel[currentOverlay.label];

        if (!currentOverlay.geojsonUrl) return;

        var languageUrl = "",
          available = false;

        if (CONFIG.LANGUAGES) available = true;
        ...
        var currentFromLocalStorage = localStorage.getItem(
          offlineConf.resourceBaseUrl + currentOverlay.geojsonUrl
        );
        ...
        var poiCallback = function (data, currentOverlay) {
          ...
          let featuresArray = [];
          if (data.type === "Feature") featuresArray.push(data);
          else if (
        ...
        var lineCallback = function (data, currentOverlay) {
          var geoJsonOptions = {
              onEachFeature: function (feature, layer) {
                if (!feature.parent) feature.parent = currentOverlay;
        ...
        if (useLocalCaching && currentFromLocalStorage) {
          ...
          overlayLayersQueueByLabel[currentOverlay.label] = $.getJSON(
            langGeojsonUrl,
            function (data) {
              if (currentOverlay.type === "line_geojson")
            ...
            overlayLayersQueueByLabel[currentOverlay.label] = $.getJSON(
              ...
              console.warn(
                "An error has occurred downloading geojson '" +
                  currentOverlay.geojsonUrl +
                  "'. This file could miss in the server or the app is offline, and will be skipped",
                err
              );
              defer.resolve();
            });
        ...
        promise.then(function () {
          ...
          if (
            ...
            (CONFIG.LANGUAGES &&
              CONFIG.LANGUAGES.actual &&
              CONFIG.LANGUAGES.actual.substring(0, 2) === currentLang &&
              lang.length !== 2)
          )
      ...
      var initializeUtfGridGeojson = function (currentOverlay) {
        ...
        var tileLayer = (tileLayersByLabel[currentOverlay.label] = L.tileLayer(
          currentOverlay.tilesUrl + "{z}/{x}/{y}.png",
          options
        ));

        utfGridOverlayLayersByLabel[currentOverlay.label] = L.utfGridCanvas(
          currentOverlay.tilesUrl + "{z}/{x}/{y}.grid.json",
          {
        ...
        utfGridOverlayLayersByLabel[currentOverlay.label].on(
          "mouseover",
          function () {
            if (mapReference) mapReference.style.cursor = "pointer";
          }
        );
        utfGridOverlayLayersByLabel[currentOverlay.label].on(
          "mouseout",
          function () {
            if (mapReference) mapReference.style.cursor = "default";
          }
        );
      ...
      var initializeLayers = function () {
        ...
        for (var i = overlayLayersConf.length - 1; i >= 0; i--) {
          if (!overlayLayersConf[i]) continue;
          ...
          if (overlayLayersConf[i].type === "utfgrid") {
            ...
            if (
              overlayLayersConf[i].type === "poi_geojson" ||
        ...
        var promiseCallback = function () {
          ...
          if (queueLayerToActivate !== null) {
            if (queueLayerToActivate === "$all") mapService.showAllLayers();
            else mapService.activateLayer(queueLayerToActivate);
      ...
      var setBaseLayer = function (baseMap, baseTms, baseLayer) {
        if (baseMap.default) {
          Offline.setDefaultInfo(
            baseLayer,
            baseMap.tilesUrl + "{z}/{x}/{y}.png",
            baseTms,
            mapService.resetView
          );
          localStorage.currentMapLayer =
      ...
      var buildBaseLayer = function (maxBounds, i) {
        return $q(function (resolve, reject) {
          ...
          if (baseMap.type === "maptile") {
            ...
            utfGridBaseLayerByLabel[baseMap.label] = L.utfGridCanvas(
              baseMap.tilesUrl + "{z}/{x}/{y}.grid.json",
              {
            ...
            utfGridBaseLayerByLabel[baseMap.label].on(
              "click",
              $.proxy(
                function (e) {
                  if (skipAreaClick) {
            ...
            if (Offline.isActive()) {
              ...
              setBaseLayer(
                baseMap,
                false,
                L.tileLayer.mbTiles(address, options)
              );
              resolve();
            } else {
      ...
      var initialize = function () {
        ...
        if (map && map !== null) return map;

        if (mapConf.layers.length === 0) return;
        ...
        if (
          CONFIG.OPTIONS.activateZoomControl ||
          (CONFIG.MAIN && CONFIG.MAIN.OPTIONS.activateZoomControl)
        ) {
          L.control
            .zoom({
              position: "topright",
        ...
        map.on("move", function () {
          ...
          etrs89coords = proj4(etrs89projection, [
        ...
        map.on("zoomend", function () {
          ...
          if (zoom === +CONFIG.MAP.maxZoom) map.doubleClickZoom.disable();
              ...
              onLocationOutsideMapBounds: function () {
                $ionicPopup.alert({
                  template: $translate.instant(
                    "Sembra che tu sia fuori dai limiti della mappa!"
                  ),
        ...
        markerClusters = new L.MarkerClusterGroup({
          ...
          showCoverageOnHover:
            mapConf.markerClustersOptions.showCoverageOnHover,
      ...
      mapService.activateLayer = function (
        layerName,
        skipReset,
        skipFit,
        skipQueueReset
          ...
          var doAdd = function () {
            ...
            while (currentHits < maxHits && layersCache.length !== 0) {
              ...
              if (!skipFit) {
                ...
                if (currentLayer) currentLayer.addTo(groupLayer);
              }
        ...
        if (
          ...
          (groupConfMap[layerName] && groupConfMap[layerName].isReady)
        ) {
          if (!skipReset) mapService.resetLayers();
        ...
        } else queueLayerToActivate = layerName;
      };
      ...
      mapService.setFilter = function (layerName, value) {
        ...
        if (!CONFIG.MAP.filters) {
          ...
          else mapService.removeLayer(layerName);
        } else {
      ...
      mapService.drawPosition = function (position) {
        if (
          ...
          circleLocation.position = L.marker(
          ...
          circleLocation.accuracy = L.circle(
            [position.coords.latitude, position.coords.longitude],
            {
              weight: 1,
              color: "#3E82F7",
      ...
      mapService.isInBoundingBox = function (lat, long) {
        ...
        return bounds.contains(new L.latLng(lat, long));
      };

      mapService.precacheOverlaysData = function () {
        for (var overlay in overlayLayersConf) {
          $.getJSON(
            overlayLayersConf[overlay].url,
            $.proxy(
              function (data) {
                setItemInLocalStorage(
                  overlayLayersConf[this.overlay].url,
                  data
                );
              },
              {
      ...
      mapService.addFeaturesToFilteredLayer = function (
        groupedFeatures,
        fitToBounds,
        delayedFit
      ) {
      ...
      mapService.getFeatureById = function (id, layerName) {
        ...
        var checkNow = function () {
          if (featureMapById[id]) defer.resolve(featureMapById[id]);
          else defer.reject();
        };
        ...
        if (
          ...
          $.getJSON(
            overlayLayersConfMap[layerName].featureUrl + id + ".geojson",
            function (data) {
          ...
          if (overlayLayersByLabel[layerName]) checkNow();
          else {
      ...
      mapService.getRelatedFeaturesById = function (relatedId) {
        ...
        for (var pos in relatedId) {
          if (featureMapById[relatedId[pos]])
            features.push(featureMapById[relatedId[pos]]);
        }

        return features;
      };

      mapService.getRelatedRicetteById = function (relatedId) {
        var features = [];
      ...
      mapService.getEventById = function (id) {
        ...
        var checkNow = function () {
          if (eventsMap[id]) defer.resolve(eventsMap[id]);
          else defer.reject();
        };

        if (eventsMap[id]) checkNow();
        else {
          setTimeout(function () {
      ...
      mapService.getCouponById = function (id) {
        ...
        var checkNow = function () {
          if (couponsMap[id]) defer.resolve(couponsMap[id]);
          else defer.reject();
        };

        if (couponsMap[id]) checkNow();
        else {
          setTimeout(function () {
      ...
      mapService.startControlLocate = function () {
        if (controlLocate !== null) controlLocate.start();
      };

      mapService.stopControlLocate = function () {
        if (controlLocate !== null) controlLocate.stop();
      };
      ...
      mapService.triggerNearestPopup = function (latLong) {
        var layerToSelectNearest = leafletKnn(
          extraLayersByLabel["filteredPOI"]
        );
        var nearPois = layerToSelectNearest.nearest(
          [latLong.long, latLong.lat],
          10,
          100
        );
      ...
      mapService.triggerFeatureClick = function (id) {
        if (map && featureMapById[id]) {
          ...
          if (feature.geometry.type === "LineString") {
            ...
            else pos = feature.geometry.coordinates.length / 2;
          ...
          setTimeout(function () {
            activatePopup(
              obj,
              feature.geometry.type === "LineString" ? false : true
            );
          }, 1000);
        }
      };
